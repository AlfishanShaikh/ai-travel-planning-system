...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { messages, isFinal, mode } = body;
    // const { messages, isFinal, mode } = await req.json();
    
  //* Rate Limit Set Feature - Start
  // const {message, isFinal } =await req.json();
   const user = await currentUser();

   // Check the Limit
   const {has} = await auth();
   const hasPremiumAccess = has({ plan: 'monthly' }) // monthly - key - in clerk - billing
  //  console.log("hasPremiumAccess", hasPremiumAccess); - give false in Terminal 
   
  //  const decision = await aj.protect(req, { userId:user?.primaryEmailAddress?.emailAddress??'', requested: isFinal ? 5 : 0 });
  const decision = await aj.protect(req, { userId: user?.id ?? 'anonymous', requested: isFinal ? 1 : 0 });
  //  console.log(decision);

   //@ts-ignore
  //  if(decision.reason?.remaining==0 && !hasPremiumAccess) {  // && !hasPremiumAccess - check have premium access or not 
...
// 🔹 Choose model smartly to avoid token limit
// const model = isFinal
//   ? "llama-3.3-70b-versatile"   // final JSON (complex)
//   : "llama-3.1-8b-instant";    // simple Q&A
...
try {
  ...
  // Groq rate-limit / busy / token exceeded
  if (err?.status === 429) {
    return NextResponse.json({
      resp: "⚠️ AI is busy right now. Please try again in a few minutes.",
      ui: "busy",
    });
  }
